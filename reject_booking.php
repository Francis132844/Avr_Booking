<?php
session_start();
include 'db.php';

if (!isset($_SESSION['admin_id'])) {
    header("Location: admin_login.php");
    exit;
}

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['reject_id'])) {
    $booking_id = intval($_POST['reject_id']);
    $rejection_message = trim($_POST['rejection_message']);

    // Get user_id from booking
    $stmt = $conn->prepare("SELECT user_id FROM bookings WHERE id = ?");
    $stmt->bind_param("i", $booking_id);
    $stmt->execute();
    $stmt->bind_result($user_id);
    $stmt->fetch();
    $stmt->close();

    if ($user_id) {
        // Update booking status and message
        $stmt = $conn->prepare("UPDATE bookings SET status = 'rejected', rejection_message = ? WHERE id = ?");
        $stmt->bind_param("si", $rejection_message, $booking_id);
        $stmt->execute();
        $stmt->close();

        // Send notification
        $notif_msg = "❌ Your booking request was rejected." . ($rejection_message ? " Reason: $rejection_message" : "");
        $notif_stmt = $conn->prepare("INSERT INTO notifications (user_id, message, is_read, created_at) VALUES (?, ?, 0, NOW())");
        $notif_stmt->bind_param("is", $user_id, $notif_msg);
        $notif_stmt->execute();
        $notif_stmt->close();
    }
}

header("Location: manage_bookings.php");
exit;
